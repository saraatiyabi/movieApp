...
export default function Index() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.themoviedb.org/3/movie/${movieId}/videos?language=en-US`, options)
      .then(response => response.json())
      .then(response => {
        setMainTrailer(`https://www.youtube.com/embed/${response.results[0].key}`)
    ...
    <div style={{ backgroundColor: "black" }}>
      {
        isLoading ? (<Spinner />) : (<div className='content'>
          <Swiper
            ...
            loop={true}
          >
            {showTrailerModal && <TrailerModal trailer={mainTrailer} setShowTrailerModal={setShowTrailerModal} isTrailerLoading={isTrailerLoading} />}
            {
              movieItems.map((item, i) => (
                <SwiperSlide key={i}>
                  <HeaderSlide {...item} media_type="movie" setShowTrailerModal={setShowTrailerModal} onMovieId={(id) => setMovieId(id)} />
                </SwiperSlide>
              ))
            }
          </Swiper>
