    ...
    useEffect(() => {
        ...
        fetch(`https://api.themoviedb.org/3/person/${personId}/movie_credits?language=en-US`, options)
            ...
            .then(response => {
                ...
                const latestCasts = casts.filter(cast => Number(cast.release_date.slice(0, 4)) > 2015 && cast.vote_average >= 5).slice(0, 5)
                setPersonCredits(latestCasts)
            })
            .catch(err => console.error(err));

        fetch(`https://api.themoviedb.org/3/person/${personId}/tv_credits?language=en-US`, options)
            .then(response => response.json())
            .then(response => {
                console.log(response)
                const casts = response.cast;
                const latestCasts = casts.filter(cast => Number(cast.first_air_date.slice(0, 4)) > 2015 && cast.vote_average >= 5).slice(0, 5)
        ...
        <div className='person-info'>
            {
                isLoading ? (<Spinner />) : (
                    <div className='person-content'>
                        <div className='row'>
                            <div className='col-3'>
                                <img className='person-img' src={`https://image.tmdb.org/t/p/w300/${personDetails.profile_path}`} />
                                                ...
                                                personCredits.reverse().map(credit => (
                                                    <SwiperSlide>
                                                        <div className='credit'>
                                                            <img src={`https://image.tmdb.org/t/p/w200/${credit.poster_path}`} />
                                                            <span>{credit.title ? credit.title : credit.name}</span>
                                                        </div>
                                                    </SwiperSlide>
                                                ))
                                            }
